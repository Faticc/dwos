-- Основные компоненты: component.gpu, component.screen и т.д. - это
-- component.getPrimary(тип). Здесь же выбор основного при подключении.
...
function component.isAvailable(componentType)
  ...
  if not primaries[componentType] and not adding[componentType] then
    -- без памяти прокси может не создаться: не пытаемся снова и снова
...
function component.setPrimary(componentType, address)
  ...
  if primary then
    if wasAvailable or wasAdding then
      -- замена основного - с небольшой задержкой, чтобы все успели отпустить старый
...
local function onComponentAdded(_, address, componentType)
  ...
  if prev then
    if componentType == "screen" then
      -- у нового экрана есть клавиатура, а у основного нет - берём новый
    ...
    elseif componentType == "keyboard" and address ~= prev.address then
      -- клавиатура становится основной, только если она первая у основного
      -- экрана, а прежняя - нет
...
local function onComponentRemoved(_, address, componentType)
  if primaries[componentType] and primaries[componentType].address == address or
    ...
    if componentType == "screen" and nxt then
      ...
      if proxy then
        -- у нового основного экрана своя клавиатура - она и главная

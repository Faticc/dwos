-- Терминал: привязка видеокарты к экрану, зажатые клавиши, смена
-- компонентов под окном терминала.
local event = require("event")
local keyboard = require("keyboard")

-- видеокарта и экран есть - связать и отдать терминалу
...
local function components_changed(ename, address, ctype)
  ...
  if ename == "component_removed" or ename == "component_unavailable" then
    ...
    elseif ctype == "keyboard" then
      window.keyboard = nil -- терминал заново выберет клавиатуру
    end
  ...
  elseif (ename == "component_added" or ename == "component_available") and ctype == "keyboard" then
    -- новая клавиатура могла встать к экрану терминала
    window.keyboard = nil
  end
end

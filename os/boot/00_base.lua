-- Основа: loadfile/dofile/print, процессы и сопрограммы, os.*, стандартные
-- потоки. Раньше это были четыре файла; один файл - одно открытие вместо
-- четырёх, а открытие файла стоит четверть бюджета тика.
...
-- process грузится через loadfile, поэтому только теперь
local process = require("process")
local _coroutine = coroutine -- настоящие сопрограммы

-- coroutine.* процесса: у процессов в канале свои resume/yield
_G.coroutine = setmetatable({
  resume = function(co, ...)
    local proc = process.info(co)
    -- процесс мог закончиться, а сопрограмма ещё жива: пусть resume решает сам
...
-- load без окружения берёт окружение текущего процесса
...
-- открытые файлы принадлежат процессу и закрываются вместе с ним

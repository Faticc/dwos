...
-- Каналы между процессами. Каждый процесс конвейера - своя стопка
-- сопрограмм; запись в канал будит следующий процесс, чтение из пустого
-- канала уступает предыдущему. yield_past(co) пробрасывает yield сквозь
-- стопку до сопрограммы co.
local pipe = {}
local _root_co = assert(process.info(), "process metadata failed to load").data.coroutine_handler

-- root - сопрограмма процесса или функция (тогда процесс создаётся)
function pipe.createCoroutineStack(root, env, name)
  ...
  function pco.resume(co, ...)
    ...
    while true do -- подряд идущие системные yield
...
local pipe_stream = {
  continue = function(self, exit)
    ...
    while true do -- B (в A|B) может уступить сам по себе, тогда снова
      if coroutine.status(self.next) == "dead" then
        self:close()
        -- канал закрылся: пишущий должен остановиться (cat огромного | head)
        result[1] = nil
      end
      ...
      if self.read_mode then
        return self -- B ждёт, пока A напишет ещё
      end
      -- B ждёт события: выпустить yield из стопки и вернуть ответ B
  ...
  read = function(self, n)
    ...
    if self.buffer == "" then
      -- read_mode отличает этот yield ("A, пиши ещё") от ожидания событий
      self.read_mode = true
...
local chain_stream = {
  read = function(self, value, ...)
    ...
    self.ready = false -- процесс канала поставит true, когда закончит
    ...
    if not self.ready then
      -- программа уступила, ничего не записав и не прочитав
...
function pipe.popen(prog, mode, env)
  ...
  -- свой процесс для ввода-вывода канала
  ...
  -- popen сразу запускает команду, как поток

...
-- Потоки: функция крутится рядом с программой и просыпается на события.
local thread = {}
local init_thread
...
local function waitForDeath(threads, timeout, all)
  ...
  while deadline > computer.uptime() do
    ...
    for _, t in ipairs(threads) do
      ...
      -- приостановленный считается мёртвым; упал процесс-хозяин - тоже
    ...
    -- все живые потоки ждут событий: дадим им проснуться
...
function box_thread:attach(parent)
  ...
  if mt.attached then
    -- подписка живёт у хозяина: снять до переезда
...
function thread.create(fp, ...)
  ...
  t.pco = pipe.createCoroutineStack(function(...)
    ...
    -- pullSignal выходит наружу через yield_past, обычный yield - сюда
  ...
  -- resume, следящий за смертью процесса
  ...
  function mt.register(timeout)
    mt.id = event.register(
      nil, -- nil ловит любое событие, у таймеров ключ false
      mt.private_resume,
      timeout,
      1, -- проснуться один раз
  ...
  function mt.coma()
    mt.unregister() -- до resume не просыпаться
    while mt.__status == "suspended" do
      t.pco.yield_past(t.pco.root, 0)
    end
  end

  -- Ожидание события в потоке: зарегистрироваться на пробуждение и выйти
  -- из стопки; разбудят - проверить, не приостановили ли нас.
  ...
  t:attach() -- к текущему процессу
  mt.private_resume(...) -- потоки стартуют сразу
...
do
  ...
  -- подписки делятся по процессам, иначе поток получал бы событие дважды
  if not handlers_mt.threaded then
    ...
    root_data.handlers = {}
    root_data.pull = handlers_mt.__call -- настоящий computer.pullSignal
    while true do

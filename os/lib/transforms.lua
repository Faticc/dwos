-- Операции над списками (как string.* над строками). Сама система их
-- больше не использует, библиотека оставлена для совместимости.
local lib = {}
lib.internal = {}
...
--- Первый диапазон в tbl, подходящий под pred (функция или список образцов).
...
--- Начинается ли tbl с позиции f поэлементно так же, как v.
...
--- Как string.sub, только по элементам таблицы.
...
--- Разрезать tbl на части; partitioner говорит, где разрез.
...
--- c(e, i, tbl) для каждого элемента; результаты - в новый список.
...
--- Пара ключ-значение под номером index в порядке pairs.

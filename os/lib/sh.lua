...
--- Слово - это ровно один незакавыченный кусок из списка vs?
...
--- Что вернула программа -> код выхода как в bash.
...
--- Разобрать строку и подставить псевдонимы в начало каждой команды.
  ...
  while i <= #words do
    ...
    elseif prev_was_delim then
      ...
      if #nxt == 1 and not nxt[1].qr then
        local key = nxt[1].txt
        if key == "!" then
          prev_was_delim = true -- после ! снова команда
        elseif not resolved[key] then
          ...
          if value and key ~= value then
            ...
            -- псевдоним разворачивается на месте этого слова
            local rest = {}
...
--- Процессы под команды конвейера; при нескольких - связать каналами.
function sh.internal.createThreads(commands, env, start_args)
  ...
  for i = 1, #commands do
    ...
    local thread, reason = process.load(program or "/dev/null", thread_env, function(...)
      ...
      -- аргументы команды + дополнительные + то, с чем процесс запустили
      local all, n = {}, 0
      ...
      -- popen ждёт от каждого процесса сначала пустую запись: так соблюдается порядок
      io.write("")
...
function sh.internal.executePipes(pipe_parts, eargs, env)
  ...
  for _, words in ipairs(pipe_parts) do
    ...
    if reparse then
      ...
      if not args then
        return false, redirects -- здесь redirects - текст ошибки
      end
    end
...
function sh.execute(env, command, ...)
  ...
  local eargs = table.pack(...) -- pack, аргументы могут идти с дырами
  -- простая команда: без ; $ & | ! < >

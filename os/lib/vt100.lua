-- Разбор escape-последовательностей vt100 прямо по строке, без перебора
-- шаблонов: смотрим на символ после ESC и на последний символ CSI.
local vt100 = {}
...
-- ESC[...m: цвета, мигание, инверсия. Пустой параметр (;; или ; по краям)
-- значит сброс, как в оригинале.
local function sgr(window, params)
  ...
  -- разделители и числа по очереди: " _ 31 _ 42 _ "
  local tokens = { "_" }
...
-- CSI: последний символ -> обработчик(окно, параметры); false - не подошло
local CSI = {
  ...
  n = function(w, p)
    ...
    -- ответ кладётся прямо во ввод; на экран он не выводится
...
--- Разобрать последовательность в buf с позиции pos (там ESC).
--- Возвращает, сколько байт съедено, и что напечатать буквально;
--- nil - последовательность ещё не дошла целиком, ждём продолжения.
function vt100.consume(window, buf, pos)
  ...
  -- не наша последовательность: ESC печатается как есть
  return 1, "\27"
end

--- Совместимость: разбор начала window.output_buffer.
--- "" - разобрано (или там не ESC), "\27" - мусор, nil - ждать продолжения.

-- Процессы: у каждой запущенной программы своя сопрограмма, окружение,
-- переменные, открытые ручки и стандартные потоки.
local process = {}

process.list = setmetatable({}, { __mode = "k" })

--- Процесс, которому принадлежит сопрограмма (своя или созданная в нём).
...
function process.load(path, env, init, name)
  ...
  thread = coroutine.create(function(...)
    -- код под pcall, чтобы при любом исходе убрать процесс из списка
    local result = {
      xpcall(function(...)
    ...
    -- обработчик ошибок открывает файл, а без процесса нельзя - закрываем последним
...
--- Гнать процесс до конца; его собственные yield уходят наверх, как
--- event.pull в духе ComputerCraft.
...
function process.running(level) -- для старых программ, лучше process.info

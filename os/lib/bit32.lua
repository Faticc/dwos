-- bit32 для Lua 5.3: в 5.2 он встроенный и этот файл не грузится.
local bit32 = {}
...
local function fold(init, op, ...)
  local result = init
  for i = 1, select("#", ...) do
    result = op(result, (select(i, ...)))
  end
  return result
end

function bit32.arshift(x, disp) return x // (2 ^ disp) end
function bit32.band(...) return fold(0xFFFFFFFF, function(a, b) return a & b end, ...) end

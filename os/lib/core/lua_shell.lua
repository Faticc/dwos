-- Интерактивный Lua: библиотеки подгружаются сами при первом обращении
-- по имени (component, fs нельзя - только полные имена модулей).
...
env = setmetatable({}, {
  ...
  __pairs = function(t)
    -- свои переменные, потом глобальные, потом загруженные модули
...
-- Tab: дописать имя поля/функции по точкам (component.gp -> component.gpu.)

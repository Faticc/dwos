-- Метки устройств для /dev/components/by-label: у кого своей метки нет,
-- та хранится в /etc/udev/rules.d/*.lua.
...
function lib.saveRule(rule_set, path)
  ...
  if not file then return end -- только для чтения - не сохранится, не беда
...
function lib.setDeviceLabel(proxy, label)
  ...
  if not irule then
    if proxy.setLabel then
      return proxy.setLabel(label) -- умеет сам
    end

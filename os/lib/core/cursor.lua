-- Строка ввода терминала: редактирование, история, подсказки по Tab.
...
-- back - сколько символов после курсора ещё идёт за вставкой
function V:update(arg, back)
  ...
  if type(arg) == "string" then
    ...
    self.data = s1 .. arg
  elseif arg then -- число: удалить назад (<0) или вперёд (>0)
...
function V:echo(arg, num)
  ...
  local gpu = win.gpu
  -- эхо идёт в тот поток, из которого читаем; ввод из канала - без эха
  ...
  elseif arg == keys.right then
    ...
    win.x, win.y = x, y
    arg = ""
  elseif not arg or arg == true then -- мигание курсора
    local char = self.char_at_cursor
    if (arg == nil and not char) or (arg and not self.blinked) then
      char = char or self:echo("") -- прокрутить и взять символ
...
function V:handle(name, char, code)
  if name == "clipboard" then
    self.cache = nil -- вставка сбрасывает подсказки
  ...
  elseif name == "key_down" then
    ...
    else self.cache = backup_cache -- служебные клавиши подсказки не сбрасывают
    end
  end
  return true
end

-- затереть введённое после курсора
V.clear = "\27[J"

function core_cursor.new(base, index)
  -- методы base вызываются первыми, методы отсюда - как super
  base = base or {}

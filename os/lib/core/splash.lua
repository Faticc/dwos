-- Заставка загрузки: логотип пикселями, полоса хода и строка состояния.
-- Рисуется через gfx: первый кадр уходит одним bitblt, дальше каждое
-- обновление - пара ячеек полосы и строка текста прямо на экран.
local gfx = ...
...
function splash.start(gpu)
  ...
  cv:clear(BG)
  -- тень, потом буквы с переливом сверху вниз
  for pass = 1, 2 do
  ...
  -- полоса хода: целая строка символов (две точки), с отступом под логотипом
...
--- Строка по центру; строка целиком перекрывает прежнюю надпись.
...
function splash:status(msg, frac)
  ...
  cv:flush(true)
  -- надписи flush затирает фоном, поэтому они кладутся заново каждый раз

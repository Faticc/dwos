-- Ядро загрузки, вызывается из /init.lua.
local raw_loadfile = ...
...
-- уровень работы: S - загрузка, 1 - работа, 0/6 - выключение/перезагрузка
_G.runlevel = "S"
...
computer.shutdown = function(reboot)
  ...
  if os.sleep then
    ...
    os.sleep(0.1) -- дать всем обработать выключение
  end
  shutdown(reboot)
end
...
-- Заставка. Не вышло (нет видеокарты, что-то сломалось) - строки текстом.
...
local function status(msg, frac)
  ...
  -- долгая загрузка не должна упасть по таймауту: уступаем и возвращаем сигнал
...
do
  -- из глобального окружения - в пакеты
...
-- загрузочные сценарии из /boot по порядку имён
local scripts = {}
...
computer.pushSignal("init") -- компоненты разосланы, можно начинать

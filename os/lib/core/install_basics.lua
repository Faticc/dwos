-- install: откуда и куда можно ставить. Возвращает таблицу с вариантами;
-- спрашивать пользователя - дело вызывающего (текстом или окном).
...
-- только диски-компоненты (не devfs, не привязки)
...
-- одна цель - она не может быть и источником
if #targets == 1 then
...
for dev, path in pairs(devices) do
  ...
  if fs.list(install_path)() and (specified or not source_filter and address ~= tmpAddress) then
    ...
    -- Корневой диск источником не бывает: с него уже загрузились. Исключение -
    -- установочный носитель: лут-дискеты read-only, а наша дискета пишется,
    -- поэтому её опознаём по .prop в корне.

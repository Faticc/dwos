-- Редко нужная часть buffer: seek, форматное чтение, построчная буферизация.
...
function buffer:buffered_write(arg)
  ...
  -- построчно: всё до последнего перевода строки уходит сразу
  local l = arg:find("\n[^\n]*$")
...
function buffer:readBytesOrChars(readChunk, n)
  ...
  while have < n do
    local needed = n - have
    -- пустой буфер - дочитать; последний байт не режем, чтобы не порвать utf-8
    if #self.bufferRead == 0 then
      ...
      if not result then
        ...
        return have > 0 and table.concat(parts) or nil -- конец
      end
    end
    ...
    if len(splice) > needed then
      ...
      if len(splice) ~= needed then
        -- поток не utf-8: лучше отдать больше, чем испортить данные

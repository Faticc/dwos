...
function term.internal.open(...)
  ...
  -- старые программы пишут window.w и window.h напрямую
  setmetatable(window, {
  ...
  -- первое окно: нынешнее tty.window становится окном корневого процесса
...
-- term.write = io.write нельзя: io.write идёт через метатаблицу

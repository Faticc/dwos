-- Копирование и перенос для cp, mv и install.
...
-- "откуда/." и "откуда/.." - копировать содержимое, а не сам каталог
...
-- Уступать раз в полсекунды, а не после каждого файла: уступка стоит тик,
-- и на сотне файлов это пять лишних секунд. Прервать всё равно можно.
...
local function prompt(message)
  ...
  if not result then -- канал закрылся
    os.exit(1)
  end
  ...
  for _, fromArg in ipairs(args) do
    ...
    if ok then
      ...
      if contents_of and options.cmd == "mv" then
        ...
        -- в существующий каталог - под своим именем (если не "содержимое")

-- Ноты для computer.beep: имя <-> MIDI <-> частота. Автор идеи - Vexatos.
local computer = require("computer")

local note = {}
local notes = {}        -- имя -> MIDI (строкой, как в оригинале)
local reverseNotes = {} -- MIDI -> имя
...
do
  ...
  -- бемоли после reverseNotes: имя по коду - всегда через диез
  for _, v in ipairs(flat) do
    notes[v] = tostring(notes[v:gsub("(.)b(.)", "%1%2")] - 1)
  end
end

--- Имя или частота -> MIDI.
...
--- Тики нотного блока (0-24) <-> MIDI (34-58).

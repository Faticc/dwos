...
-- таблицы с отложенной загрузкой (__pairs) обходятся через свой __pairs
...
--- В строку Lua. pretty - для людей: несериализуемое показывается как есть,
--- но обратно такое уже не читается.
function serialization.serialize(value, pretty)
  ...
  local function recurse(v, depth)
    ...
    elseif t == "table" then
      ...
      local f
      if pretty then
        -- для людей ключи по порядку: числа, строки, остальное
        local ks, sks, oks = {}, {}, {}
      ...
      ts[v] = nil -- одну таблицу можно записать не раз
      emit("}")
    else

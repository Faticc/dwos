...
-- События: подписчики, таймеры и ожидание сигналов. Каждое ожидание
-- (computer.pullSignal) заодно раздаёт сигнал всем подписчикам.
local event = {}
local handlers = {}
...
computer.pullSignal = function(seconds)
  ...
  repeat
    ...
    -- копия: обработчики могут менять список, пока мы по нему идём
    local copy = {}
    for id, handler in pairs(handlers) do
      copy[id] = handler
    end
    for id, handler in pairs(copy) do
      -- у таймеров ключ false, ключ nil ловит всё
      if handler.key == nil or handler.key == signal or uptime() >= handler.timeout then
        ...
        -- убрать до вызова: таймер может сам ждать событий
...
-- event.push привычнее, чем computer.pushSignal

-- luascan: разбор исходника Lua для редактора. Не полный парсер, а проход
-- по лексемам с учётом блоков и областей видимости - этого хватает, чтобы
-- знать, какое имя что значит:
--
--   lexer(src)          -> next(): лексемы { t = тип, v = текст, l = строка, c = байт в строке }
--   analyze(src, opt)   -> {
--     diags   = { {l, c, len, msg} }       подозрительное: неизвестное имя,
--                                          глобальная внутри функции, лишний local
--     funcs   = { {name, l, depth} }       функции файла, для списка
--     blocks  = { {first, last} }          многострочные блоки, для свёртки
--     globals = { имя = {l, c} }           где глобальная определена
--     refs    = { {l, c, len, name, decl, write} }  упоминания имени opt.name
--     decls   = { {l, c, name, kind} }     его объявления; decl у ссылки - одно из них
--     hooks   = { [строка] = { c, at } } с opt.hooks: строки, в начало которых
--                                          можно вставить вызов; names(at) - видимые там local
--   }
--
-- opt.known(имя) - есть ли такая глобальная (редактор смотрит в _G).
--
-- Память в OC дорогая, поэтому лексемы не собираются в список: они идут
-- потоком, а разбор держит только несколько впереди и позади. Ссылки
-- копятся лишь на одно имя - то, что под курсором.
...
--- Лексемы исходника по одной. Комментарии пропускаются, строки и числа -
--- одной лексемой (текст строки не хранится: он разбору не нужен).
function M.lexer(src)
  local i, len, line, ls = 1, #src, 1, 1
  -- пропустить длинную скобку до её конца, считая строки
          ...
          while j <= len do
            ...
            if d == 92 then
              ...
              if src:byte(j + 1) == 122 then
                -- \z съедает пробелы и переводы строк после себя
...
-- Лексемы, после которых может начаться новый оператор: перед следующей
-- строкой тогда можно вставить свой вызов. После остальных (операторы,
-- запятая, открытая скобка, local, return...) выражение ещё не кончилось.
local ENDS = { ["then"] = true, ["do"] = true, ["else"] = true, ["repeat"] = true,
  ["break"] = true, ["end"] = true, ["true"] = true, ["false"] = true, ["nil"] = true,
  [")"] = true, ["]"] = true, ["}"] = true, ["..."] = true, [";"] = true }
-- С чего строка может начинаться, чтобы вставка перед ней была оператором.
...
-- Эти лексемы точно начинают новый оператор: отложенные local вступают в силу.
...
function M.analyze(src, opt)
  ...
  -- поток лексем: ahead[1] - текущая, дальше - заглянутые вперёд;
  -- back - несколько пройденных, для имени функции перед "= function"
  ...
  -- scope: { vars = {имя -> decl}, list, fn = начало функции, ret = после return,
  --          pend = переменные цикла, ждущие своего do }
  local scope = { vars = {}, fn = true, list = {} }
  local scopes = { scope }
  local cons = {}               -- if/for/while/function/do/repeat: { l, kind }
  local brk = {}                -- открытые скобки текущей функции
  local brkStack = {}
  local pending, pendingLine    -- local a, b = ... : вступят в силу со следующего оператора
  local lingering               -- local из repeat видны в условии until
  local fnDepth = 0
  local globalWrites, globalReads = {}, {}
  local known = opt.known or function() return false end
  -- видимые local - цепочка: у каждого объявления ссылка на прошлое видимое.
  -- Строке для отладчика хватает запомнить звено, списки не копируются
  local head
  ...
  -- later: local из repeat проверяются после until, они видны в его условии
  ...
  --- Параметры функции: лексема n впереди - "(". Возвращает, сколько пройти.
  ...
  -- Имя функции для списка по тому, что стоит перед "function": a.b = function.
  ...
  while true do
    ...
    -- первая лексема строки: можно ли вставить перед ней вызов
  ...
  -- глобальные: чего нет ни в окружении, ни в файле - скорее опечатка
  local everWritten = {}
...
--- Имена local, видимых в звене at цепочки: ближние первыми, без повторов.
...
--- Ссылка на переменную в строке l, байт c (курсор внутри имени или сразу за ним).
...
--- Все места, где стоит та же переменная, что и в ref (объявление тоже).
function M.occurrences(R, ref)
  ...
  -- объявление могло попасть и ссылкой (function f() у глобальной)
  local uniq = {}

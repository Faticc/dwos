...
-- Буферизованный поток поверх сырого (файл, tty, сокет, канал).
local buffer = {}
...
function buffer.new(mode, stream)
  ...
  -- закрыли поток - сначала закрывается буфер; закрыли буфер - потом поток
  stream.close = setmetatable({ close = stream.close, parent = result }, { __call = buffer.close })
  return setmetatable(result, metatable)
end

function buffer:close()
  -- self - либо сам буфер, либо обёртка stream.close
  if getmetatable(self) == metatable.__metatable then
...
local function readChunk(self)
  ...
  return result, reason -- ошибка или конец
end
...
function buffer:readLine(chop, timeout)
  ...
  while true do
    ...
    if not result then
      ...
      result = #self.bufferRead > 0 and self.bufferRead or nil -- конец
      self.bufferRead = ""
      return result
    end
  end
...
function buffer:write(...)
  ...
  local direct = self.bufferMode == "no"
  for i = 1, args.n do
    local result, reason
    if direct then
      result, reason = self.stream:write(args[i])
    else
      result, reason = buffer.buffered_write(self, args[i])
    end
    if not result then
      return nil, reason
    end

...
-- Дерево монтирования: узлы {name, parent, children, links, fs}. Файловые
-- системы висят на узлах, ссылки - строки в links родителя.
local filesystem = {}
  ...
  while index <= #parts do
    ...
    if not node.children[part] then
      ...
      if link_path then
        ...
        -- начало пути сохраняется: ссылка может уводить вверх через ..
        visited[path] = index
...
function filesystem.mount(fs, path)
  ...
  if not fsnode then
    ...
    fs.fsnode = fsnode -- файловая система может сама вести свои узлы (devfs)
  else
    -- та же система в другом месте: узел-двойник
...
function filesystem.proxy(filter, options)
  ...
  if not component.list("filesystem")[filter] or next(options or {}) then
    -- неполный адрес, метка или параметры - нужен полный вариант
...
function filesystem.exists(path)
  ...
  if not vrest or vnode.links[vrest] then -- каталог монтирования или ссылка
    return true
...
function filesystem.isDirectory(path)
  ...
  if not vnode.fs and not vrest then
    return true -- каталог монтирования
  end
  if node.fs then
...
function filesystem.list(path)
  ...
  if node then
    ...
    -- vrest пуст - значит vnode и есть этот каталог: добавить точки
    -- монтирования и ссылки
    if not vrest then
...
function filesystem.open(path, mode)
  ...
  -- поток: методы файловой системы с подставленной ручкой

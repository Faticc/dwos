-- less: просмотрщик. Текст читается из cat, поэтому одинаково работают
-- и файл, и канал. Экран целиком собирается в видеопамяти (gfx.surface),
-- так что прокрутка стоит один bitblt вместо полусотни set.
--
-- less --noback - это more: без буфера, текст просто идёт по терминалу.
...
-- длинную строку режем по ширине экрана, табуляции разворачиваем заранее
...
--- Дочитать поток, пока строк не станет n (или пока он не кончится).
...
-- Без буфера назад смысла в полноэкранном окне нет: пусть текст остаётся
-- в истории терминала, как у настоящего more.
if ops.noback then
  local shown = 0
  local function forward(n)
    ...
    for _ = 1, n do
      ...
      shown = shown + 1
      print(line)
      lines[shown] = nil -- назад не листаем, память не держим
    end
    return true
  end
...
--- Строка с клавиатуры прямо в служебной строке (для поиска).

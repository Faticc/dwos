-- grep на шаблонах Lua (не POSIX), по мотивам grep от Wobbo.
...
-- регистр: каждая буква шаблона -> [aA] (% и не-буквы как есть)
if ignore_case then
  for i = 1, #PATTERNS do
...
-- строки всех файлов подряд: (meta, файл) или (false, итог файла / код)
...
for meta, status in readLines() do
  ...
  if not meta then
    if type(status) == "table" then
      if flush then flush(status) end -- файл кончился
    elseif status then
      ec = status or ec
    end
  else

...
if #args > 0 then
  return sh.execute(...) -- sh команда: выполнить и выйти
end
...
while true do
  if io.stdin.tty and io.stdout.tty then
    if not has_profile then -- первый интерактивный запуск
      has_profile = true
  ...
  elseif command == nil then -- false - ввод прервали, nil - конец
    return
  end
end

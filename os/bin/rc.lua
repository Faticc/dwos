-- rc: службы из /etc/rc.d. Без аргументов - запустить включённые
-- (так делает загрузка), иначе rc <служба> [команда] [аргументы].
local rc = require("rc")
...
if select("#", ...) == 0 then
  -- при загрузке ошибки уходят в журнал событий, а не на экран

-- ps: дерево процессов. Смотрит во внутренности process и event - при их
-- изменении правится вместе с ними.
...
local cols = {
  ...
  { "PARENT", function(_, p)
    -- у потока родитель - процесс, к которому он прикреплён
...
-- дерево: дети под родителем, со сдвигом
local sorted, used = {}, {}

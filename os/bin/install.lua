-- install: перенос системы с одного диска на другой.
--
-- Разбор параметров и списки дисков - в lib/core/install_basics.lua,
-- диалоги - в lib/core/install_ui.lua. Здесь остаётся сама логика:
-- выбрать источник и цель, собрать задание для transfer, скопировать.
...
-- окно рисуем, только если оба конца терминала - настоящий терминал
...
-- noclobber: файлы, которые нельзя затирать поверх уже стоящих
...
-- .install на источнике: диск сам знает, как ставиться
...
if ui.graphic then
  -- полоса хода нужна в долях: сначала узнать, сколько всего файлов
  ui.progress(0)
